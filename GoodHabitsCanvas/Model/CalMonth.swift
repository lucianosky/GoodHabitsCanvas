...
struct CalMonth {
    ...
    let weeks: Int
    let startOfWeek: Int
    private(set) var calWeeks: [CalWeek]
    ...
    init(year: Int, month: Int, startOfWeek: Int = 1) {
        ...
        // TODO: optional
        firstMonthDate = Date.fromComponents(year, month)!
        ...
        // compute calWeeks
        calWeeks = [CalWeek]()
        calWeeks.append(CalWeek(isHeader: true, days: getWeekHeaders()))
        for week in 0..<weeks {
            calWeeks.append(CalWeek(isHeader: false, days: getWeekDays(week)))
        }
    }
